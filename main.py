...
def settings():
    ...
    def settingsLoad():
        try:
            ...
            Label(errorScreen, text= "Please enter integer value into settings", bg='red', font=('Courier 38 bold')).grid(column=0, row=1, sticky=EW)
            Label(errorScreen, text= "Might need to also be a positive integer", bg='red', font=('Courier 15 bold')).grid(column=0, row=2, sticky=S)
    ...
    # Initialize a Label
    label=Label(root, text="Amount of Cards in a hand:", bg=bgcolour, font=("Courier 22 bold"))
    label.pack()
...
def play(hands, numCards, numDecks):
    ...
    loadingLabel = tk.Label(root, text="Loading...", bg=bgcolour, font=('verdana', 40), fg='black') # If user is on a slow pc the loading label should say loading, So the user knows something is happening.
    loadingLabel.place(x=100, y=80) # Places label at these pixels
    loadingLabel["text"] = "Choose game file."  # Changes the label to choose game file when the program is ready to load a game file
    loadingLabel.place(x=10, y=80) # Places label at these pixels

    main_file = fd.askopenfile()
    file = main_file.readline() #Reads the first line in the file to then load in the text file of the deck hand. 
    file = file[:-1]
    ...
    # Compresses the carDeck list so you can split it up into hands.
    cardDeckStr = ' '.join([str(elem) for elem in cardDeck])
    cardDeckStr = cardDeckStr.replace(" ", "")
    cardHandDecks = [cardDeckStr[i*numCards*n:(i*numCards*n)+numCards*n] for i in range(hands)]
    print(cardDeck)
    print(cardDeckStr)
    print(cardHandDecks)
    root.destroy()
    ...
    def sel(i): # Changes the label to tell user what hand is showing.
        ...
        player = i-1
        card = 0
        handLabel.set("You selected hand number " + str(i))
        ...
        while card < numCards:
            try:
                print("dfs")
                print(player)
                localCardHandDecks = [cardHandDecks[player][x:x+n] for x in range(0, len(cardHandDecks[player]), n)]
                print(localCardHandDecks)
            except IndexError:
                print(IndexError)
                pass
            cardImg = (Image.open(f"cards/{localCardHandDecks[card]}.gif"))
            cardImg = cardImg.resize((108 ,151)) # Half's the size of the large playing cards. For the loading screen this is cause we don't need it to be large3
            cardImg = ImageTk.PhotoImage(cardImg)
            cardImg.im = cardImg
            card += 1
            button=tk.Button(cardsFrame, image=cardImg, command=lambda i=card: print(i), bg=bgcolour)
    ...
    while handsDealt < hands:
        ...
        handLabel.set("What hand to display?")
        handsButton = Radiobutton(root, text="Show hand " + str(handsDealt), value= handsDealt, command=lambda i=handsDealt: sel(i))
